    ...
    case ENV_CHAR_UUID:
      for ( i = 0 ; i < CONNECTION_COUNT ; i++)
      {
        if (BleApplicationContext.BleApplicationContext_legacy.ConnUpdateContext[i].connectionHandle != 0xFFFF)
        {
          do 
          {
            result = aci_gatt_update_char_value_ext(BleApplicationContext.BleApplicationContext_legacy.ConnUpdateContext[i].connectionHandle, 
                                                    aMotenvContext.HWSvcHdle,
                                                    aMotenvContext.HWEnvCharHdle,                                                                                 
                                                    1, /* notify */
            ...
            if (result != 0)
            {
              APP_DBG_MSG("Conn: 0x%04x update char ENV fails: 0x%02x\n", BleApplicationContext.BleApplicationContext_legacy.ConnUpdateContext[i].connectionHandle, result);
            }
          }while(result != 0);
        }
      }
      ...
      for ( i = 0 ; i < CONNECTION_COUNT ; i++)
      {
        if (BleApplicationContext.BleApplicationContext_legacy.ConnUpdateContext[i].connectionHandle != 0xFFFF)
        {
          do 
          {
            ...
            if (result != 0)
            {
              APP_DBG_MSG("Conn: 0x%04x update char MOTION fails: 0x%02x\n", BleApplicationContext.BleApplicationContext_legacy.ConnUpdateContext[i].connectionHandle, result);
            }
          }while(result != 0);
        }
      }
      ...
      for ( i = 0 ; i < CONNECTION_COUNT ; i++)
      {
        if (BleApplicationContext.BleApplicationContext_legacy.ConnUpdateContext[i].connectionHandle != 0xFFFF)
        {
          do{
            ...
            if (result != 0)
            {
              APP_DBG_MSG("Conn: 0x%04x update char MOTION_FX fails: 0x%02x\n", BleApplicationContext.BleApplicationContext_legacy.ConnUpdateContext[i].connectionHandle, result);
            }
          }while(result != 0);
        }
      }
