...
static uint8_t MOTION_stopDisplayValue;
...
void MOTION_Set_Notification_Status(uint8_t status)
{
  ...
  if(status == 0){
    MOTION_stopDisplayValue = 1;
  }else{
    MOTION_stopDisplayValue = 0;
  }
}
...
static void MOTION_Handle_Sensor(void)
{
  ...
  if(MOTION_Server_App_Context.hasAcc == 1)
  {
    ...
    MOTION_Server_App_Context.acceleration.x = (int32_t)((float)accelerationRaw.x * ISM330DHCX_ACC_SENSITIVITY_FS_2G);
    MOTION_Server_App_Context.acceleration.y = (int32_t)((float)accelerationRaw.y * ISM330DHCX_ACC_SENSITIVITY_FS_2G);
    MOTION_Server_App_Context.acceleration.z = (int32_t)((float)accelerationRaw.z * ISM330DHCX_ACC_SENSITIVITY_FS_2G);
  }
  ...
  if(MOTION_Server_App_Context.hasGyro == 1)
  {
    ...
    MOTION_Server_App_Context.angular_velocity.x = (int32_t)((float)angular_velocityRaw.x * ISM330DHCX_GYRO_SENSITIVITY_FS_2000DPS);
    MOTION_Server_App_Context.angular_velocity.y = (int32_t)((float)angular_velocityRaw.y * ISM330DHCX_GYRO_SENSITIVITY_FS_2000DPS);
    MOTION_Server_App_Context.angular_velocity.z = (int32_t)((float)angular_velocityRaw.z * ISM330DHCX_GYRO_SENSITIVITY_FS_2000DPS);
  }
  ...
  MOTION_stopDisplayValue = 1;
  if(MOTION_stopDisplayValue == 0)
  {
    if(MOTION_Server_App_Context.hasAcc == 1)
    {
      accX = (float)MOTION_Server_App_Context.acceleration.x;
      accY = (float)MOTION_Server_App_Context.acceleration.y;
      accZ = (float)MOTION_Server_App_Context.acceleration.z;
      sprintf(accValue,"%5.0f|%5.0f|%5.0f",accX,accY,accZ);
    }
    
    if(MOTION_Server_App_Context.hasGyro == 1)
    {    
      gyroX = (float)((MOTION_Server_App_Context.angular_velocity.x) / 100.0);
      gyroY = (float)((MOTION_Server_App_Context.angular_velocity.y) / 100.0);
      gyroZ = (float)((MOTION_Server_App_Context.angular_velocity.z) / 100.0);
